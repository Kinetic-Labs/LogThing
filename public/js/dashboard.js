document.addEventListener("DOMContentLoaded", async () => {
  ...
  const logKinds = await fetch("/api/logs/levels").then((response) =>
    response.json()
  );
  const logCounts = {};

  globalThis.activeFilters = ["ALL"];
  ...
  new Chart(ctx, {
    ...
    data: {
      ...
      datasets: [{
        ...
        backgroundColor: Object.keys(logCounts).map((level) =>
          getLevelColor(level)
        ),
        borderColor: Object.keys(logCounts).map((level) =>
          getLevelColor(level)
        ),
        borderWidth: 1,
        borderRadius: 4,
    ...
    run: (newItems) => {
      ...
      newItems.forEach((item) => {
        if (!item.element) {
          ...
          div.style.borderLeftColor = getLevelColor(
          ...
          div.innerHTML = `
              ...
              ? `
                ...
                item.data.timestamp
                  ...
                  : ""
                            ...
                            ${
                item.data.tag
                  ? `<span class="log-tag">${item.data.tag}</span>`
                  : ""
              }
                        </span>
                        `
            ...
            getLevelColor(item.data.level || "UNKNOWN")
          }">
                            [${item.data.level || "UNKNOWN"}]
                        </span>
...
function filterLogs(level) {
  ...
  filterButtons.forEach((button) => {
    button.classList.toggle(
      "active",
      activeFilters.includes(button.dataset.level),
    );
  });

  if (globalThis.updateVScroll) {
    globalThis.updateVScroll();
  }
}
